...
func TestDetectDelimiterTableDriven(t *testing.T) {
	tests := []struct {
		...
		{name: "multi-line tally", text: "a;b;c\na,b,c,d,e,f,g", want: ','},
		{name: "title row then semicolon header", text: "Depotübersicht vom 17.05.2026\na;b;c;d\n1;2;3;4", want: ';'},
	}
...
func TestComputeHash(t *testing.T) {
	...
	t.Run("ContentHash excludes AccountID", func(t *testing.T) {
		txn1 := Transaction{AccountID: accountID, Date: date, Amount: 100.0, Type: "buy"}
		txn2 := Transaction{AccountID: uuid.New(), Date: date, Amount: 100.0, Type: "buy"}
		if ContentHash(txn1) != ContentHash(txn2) {
			t.Error("ContentHash should be the same regardless of account")
		}
	})
}
...
func TestParseCSVEdgeCases(t *testing.T) {
	t.Run("empty CSV", func(t *testing.T) {
		_, _, _, err := ParseCSV([]byte(""), uuid.New())
		if err == nil {
...
func TestScalableCapitalDetect(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "native scalable format", header: []string{"date", "status", "type", "sub_type", "side", "isin", "description", "quantity", "amount", "currency", "is_cancellation"}, want: true},
		{name: "minimal required cols", header: []string{"isin", "sub_type", "is_cancellation"}, want: true},
		{name: "missing is_cancellation", header: []string{"date", "isin", "sub_type", "type"}, want: false},
		{name: "missing isin", header: []string{"date", "sub_type", "is_cancellation"}, want: false},
		{name: "unrelated csv", header: []string{"date", "isin", "amount"}, want: false},
	}
