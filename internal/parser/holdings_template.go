...
func (p *HoldingsTemplateParser) Parse(records [][]string, accountID uuid.UUID) ([]Transaction, error) {
	if len(records) < 2 {
	...
	if isinCol < 0 || quantityCol < 0 || marketValueCol < 0 {
		return nil, fmt.Errorf("missing required columns: isin, quantity, market_value")
	}
	...
	for i, record := range records[1:] {
		...
		if err != nil {
			return nil, fmt.Errorf("row %d: parse quantity %q: %w", i+2, quantityStr, err)
		}

		marketValue, err := parseStandardDecimal(marketValueStr)
		if err != nil {
			return nil, fmt.Errorf("row %d: parse market_value %q: %w", i+2, marketValueStr, err)
		}
