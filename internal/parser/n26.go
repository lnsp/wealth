...
func (p *N26Parser) Parse(records [][]string, accountID uuid.UUID) ([]Transaction, []RSUVest, error) {
	if len(records) < 2 {
	...
	if dateCol < 0 || amountCol < 0 {
		return nil, nil, fmt.Errorf("missing required columns: date and/or amount")
	}
