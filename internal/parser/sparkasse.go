...
func (p *SparkasseParser) Parse(records [][]string, accountID uuid.UUID) ([]Transaction, []RSUVest, error) {
	if len(records) < 2 {
	...
	if !hasDate || !hasAmount {
		return nil, nil, fmt.Errorf("missing required columns: buchungstag and/or betrag")
	}
	...
	for i, record := range records[1:] {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("row %d: parse amount %q: %w", i+2, amountStr, err)
		}
